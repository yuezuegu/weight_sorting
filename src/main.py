...
for i, im_name in enumerate(im_list):
    logfile.write("index: " + str(i) + " image name: " + im_name + "\n")
    ...
    for idx, layer in enumerate(model.layers):
        ...
        if layer.name.__eq__("block1_conv1"):
            ...
            ofmap, mac_cnt, ws_cnt = helpers.fcSorted(next_layer_input[0,:], weights, biases)
    ...
    top1correct, top5correct = helpers.check_accuracy(pred[0], img_ids[i])
    org_top1_cnt = org_top1_cnt + top1correct
    org_top5_cnt = org_top5_cnt + top5correct

    top1correct, top5correct = helpers.check_accuracy(pred_ws[0], img_ids[i])
